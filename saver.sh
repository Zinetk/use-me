...
# Allow ldap traffic
iptables -A OUTPUT -p tcp --dport 389 -d 172.20.241.27 -m state --state new -j ACCEPT
iptables -A OUTPUT -p tcp --dport 636 -d 172.20.241.27 -m state --state new -j ACCEPT
...
# Allow ssh traffic
iptables -A OUTPUT -p tcp --dport 22 -d 172.20.240.0/23 -m state --state new -j ACCEPT

# Allow dns traffic
iptables -A OUTPUT -p udp --dport 53 -m state --state new -j ACCEPT

# Allow ntp traffic
iptables -A OUTPUT -p udp --dport 123 -d 172.20.240.0/23 -m state --state new -j ACCEPT

# Allow rsyslog traffic to send logs
iptables -A OUTPUT -p udp --dport 514 -d 172.20.240.0/23 -m state --state new -j ACCEPT
...
echo 'deb-src http://archive.debian.org/debian-security/ squeeze/updates main contrib non-free' >> /etc/apt/sources.list
...
a2enmod rewrite
a2enmod headers
echo 'ServerName mail.team.local' > /etc/apache2/httpd.conf
echo 'RewriteEngine on' >> /etc/apache2/httpd.conf
echo 'RewriteCond %{REQUEST_METHOD} ^{TRACE|TRACK}' >> /etc/apache2/httpd.conf
echo 'RewriteRule .* - [F]' >> /etc/apache2/httpd.conf
echo 'Header edit Set-Cookie ^(.*)$ $1;HttpOnly' >> /etc/apache2/httpd.conf
echo 'TraceEnable off' >> /etc/apache2/httpd.conf
sed -i 's_Include /etc/squ_#Include /etc/squ_' /etc/apache2/apache2.conf
